...
pub fn check_cross_deadlock(crosses: &[usize], board: &Board) -> bool {
    if crosses.is_empty() { return false; }
    ...
    // 2x2 block check
    for &(r, c) in set.iter() {
        if c + 1 < board.row_widths[r] && r + 1 < board.rows
            && c < board.row_widths[r + 1] && c + 1 < board.row_widths[r + 1]
        {
            if set.contains(&(r, c + 1)) && set.contains(&(r + 1, c)) && set.contains(&(r + 1, c + 1)) {
                return true;
            }
        }
    }
